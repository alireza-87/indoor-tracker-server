...
let connection = function Broker() {
      ...
      if (packet.topic.match(dashboardCommandPattern)) {
        ...
        switch (data.type) {
          ...
          case "/App/GET_ROOM_LIST":
            storage.getAllScanners((err, res) => {
              if (err == null) {
                ...
                server.publish({
                  topic: topic,
                  payload: JSON.stringify({ type: "roomList", result: res }),
                });
              }
            });
            break;
          ...
          case "/App/GET_PERSON_LIST_OF_ROOM":
            storage.getCurrentClientInRoom(
              data.floor,
              data.room,
              (err, res) => {
                if (err == null) {
          ...
          case "/App/GET_ROOM_COUNT":
            storage.getCurrentCountOccupideInRoom(
              data.floor,
              data.room,
              (err, res) => {
                if (err == null) {
  ...
  server.on("subscribed", function (topic, client) {
    if (client != null) {
      let dibrisRoomPattern = new RegExp(
        /^\/dibrisbuilding\/[0-9]*\/[0-9]*\/[0-9A-Z-]*$/
      );
      let dibrisEntrancePattern = new RegExp(
        /^\/dibrisbuilding\/[0-9]*\/[0-9A-Z-]*\/[0-9A-Z-]*$/
      );
        ...
        storage.insertClient(floor, room, clientId, Date.now(), () => {
          storage.getCurrentCountOccupideInRoom(floor, room, (err, res) => {
            if (err == null) {
              ...
              console.log(
                JSON.stringify({ type: "roomCount", result: answer })
              );
              server.publish({
                topic: topic,
